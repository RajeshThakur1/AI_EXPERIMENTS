...
def create_user_profile(access_token, preferences, interests):
    ...
    response = requests.post(f"{API_URL}/profile", json={"preferences": preferences, "interests": interests}, headers=headers)
    if response.status_code == 200:
...
def update_user_profile(access_token, preferences, interests):
    headers = {"Authorization": f"Bearer {access_token}"}
    response = requests.put(f"{API_URL}/profile", json={"preferences": preferences, "interests": interests}, headers=headers)
    if response.status_code == 200:
        return response.json()['message']
    else:
        return f"Error: {response.json()['detail']}"
...
with gr.Blocks() as demo:
    gr.Markdown("# Hyper Personalized Chatbot System")
    ...
    with gr.Tab("Register/Login"):
        with gr.Group():
            ...
            register_button.click(register_user, inputs=[username_input, email_input, password_input], outputs=register_output)
    ...
    with gr.Tab("User Profile"):
        ...
        create_profile_button.click(create_user_profile, inputs=[access_token_input, preferences_input, interests_input], outputs=profile_output)
        update_profile_button.click(update_user_profile, inputs=[access_token_input, preferences_input, interests_input], outputs=profile_output)
        get_profile_button.click(get_user_profile, inputs=[access_token_input], outputs=profile_output)

    with gr.Tab("Chat"):
        chat_access_token_input = gr.Textbox(label="Access Token")
        chatbot = gr.Chatbot()
        msg = gr.Textbox(label="Message")
        clear = gr.Button("Clear")

        msg.submit(chat, inputs=[msg, chatbot, chat_access_token_input], outputs=[msg, chatbot])
